...
class CameraThread(threading.Thread):
    def __init__(self, name, capture_func, publisher, logger, detection_interval=10):
    ...
    def run(self):
        while self.running:
            ...
            frame_bgr = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR) if len(frame.shape) == 2 else frame
            frame_display = cv2.resize(frame_bgr, (320, 240))
            camera_frames[self.name] = frame_display

            # === Update real-time FPS every second ===
            self.frame_counter += 1
            now = time.time()
            if now - self.last_fps_time >= 1.0:
                fps_value = self.frame_counter / (now - self.last_fps_time)
                camera_fps[self.name] = f"{fps_value:.1f}"
                self.frame_counter = 0
                self.last_fps_time = now

            # === Barcode detection at interval ===
            if (now - self.last_detection_time) >= self.detection_interval:
                ...
                for data, btype in detections:
                    ...
                    fps_at_detection = 1 / (time.time() - loop_start + 1e-6)
                    duration = round(time.time() - detection_start, 2)
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

                    with open(CSV_PATH, "a", newline="") as f:
                        writer = csv.writer(f)
                        writer.writerow([timestamp, data, btype, self.name, f"{duration:.2f} s", f"{fps_at_detection:.2f}"])

                    self.publisher.publish(String(data=f"[{self.name}] {data} ({btype}) FPS: {fps_at_detection:.2f}"))
...
class ThreeCameraBarcodeNode(Node):
    def __init__(self):
        ...
        self.picam2.set_controls({"Contrast": 1.0, "Sharpness": 1.0, "Saturation": 0.0, "Brightness": 0.5})
        self.picam2.start()
        time.sleep(1)
        ...
        self.threads = [
            CameraThread("Cam1-C270", self.capture_cam1, self.publisher_1, self.get_logger(), 10),
            CameraThread("Cam2-Brio", self.capture_cam2, self.publisher_2, self.get_logger(), 10),
            CameraThread("Cam3-PiCam3", self.capture_picam3, self.publisher_3, self.get_logger(), 10),
        ]
        for t in self.threads:
            t.start()

    def capture_cam1(self): return self.cap1.read()[1] if self.cap1.isOpened() else None
    def capture_cam2(self): return self.cap2.read()[1] if self.cap2.isOpened() else None
    def capture_picam3(self): return self.picam2.capture_array()
...
class BarcodeGUI(ctk.CTk):
    def __init__(self, node):
        ...
        self.node = node
        self.camera_labels, self.fps_labels = {}, {}
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        fps_frame.pack(fill="x", padx=10)
        for name in camera_fps:
            lbl = ctk.CTkLabel(fps_frame, text=f"{name} FPS: --", anchor="w")
            lbl.pack(fill="x", padx=5)
            self.fps_labels[name] = lbl
    ...
    def update_gui(self):
        for name in self.camera_labels:
            frame = camera_frames.get(name)
            if frame is not None:
                img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img = Image.fromarray(img).resize((400, 300))
                img_tk = ImageTk.PhotoImage(image=img)
                self.camera_labels[name].img_tk = img_tk
                self.camera_labels[name].configure(image=img_tk)
        for name, label in self.fps_labels.items():
            label.configure(text=f"{name} FPS: {camera_fps.get(name, '--')}")
